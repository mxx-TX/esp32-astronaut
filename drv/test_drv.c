...
static void test_drv_touch_read(esp_lcd_touch_handle_t dev)
{
    ESP_LOGI(TAG, "========== Touch ==========");
    ESP_LOGI(TAG, "please touch (timeout:100)...");
    int timeout = 100;
    while (timeout--) {
        drv_touch_data_t t;
        esp_err_t r = drv_touch_read(dev, &t);
        if (r == ESP_OK && t.pressed) {
            ESP_LOGI(TAG, "[PASS] have! x=%u y=%u", t.x, t.y);
            return;
        }
        if (r != ESP_OK && (timeout % 50 == 0)) {
            ESP_LOGW(TAG, "touch read retrying: 0x%x", r);
        }
        vTaskDelay(pdMS_TO_TICKS(30));
    }
    ESP_LOGW(TAG, "[WARN] no touch");
}
