...
esp_err_t drv_lcd_init(const drv_lcd_cfg_t *cfg, esp_lcd_panel_io_handle_t *out_io, esp_lcd_panel_handle_t *out_panel)
{
    ...
    if (ret != ESP_OK) { ESP_LOGE(TAG, "SPI init fail: 0x%x", ret); return ret; }
    ...
    esp_lcd_panel_io_spi_config_t io_cfg = {
        ...
        .lcd_cmd_bits = 32,
        .lcd_param_bits = 8,
        .flags = { .quad_mode = true, .sio_mode = false },
    };
    ret = esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)cfg->spi_host, &io_cfg, out_io);
    if (ret != ESP_OK) { ESP_LOGE(TAG, "panel IO fail: 0x%x", ret); goto err_spi; }
    ...
    ledc_channel_config_t ledc_ch = {
        .gpio_num = cfg->bl,
        .speed_mode = LEDC_LOW_SPEED_MODE,
        .channel = cfg->bl_channel,
        .timer_sel = cfg->bl_timer,
        .duty = 0,
        .hpoint = 0,
    };
    ret = ledc_channel_config(&ledc_ch);
    if (ret != ESP_OK) { ESP_LOGE(TAG, "LEDC channel fail: 0x%x", ret); goto err_ledc; }
    return ESP_OK;
...
esp_err_t drv_lcd_set_backlight(const drv_lcd_cfg_t *cfg, uint8_t percent)
{
    ...
    ledc_set_duty(LEDC_LOW_SPEED_MODE, cfg->bl_channel, duty);
    return ledc_update_duty(LEDC_LOW_SPEED_MODE, cfg->bl_channel);
}
